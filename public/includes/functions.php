<?php
    ...
    function checkUser($uid,$oauth_provider,$username = null , $fullname = null , $email = null ,$twitter_otoken = null,$twitter_otoken_secret = null) {
		...
		if ( $oauth_provider == 'twitter' ){
			...
			$query = $db->fetchRow("SELECT * FROM `users` WHERE facebook='$uid'");
		...
		if ( $query['username'] != '' ) {
			...
			header('Location: index.php?view=home&oauth_provider='.$oauth_provider.'&uid='.$uid.'&username='.$username.'&fullname='.$fullname.'&email='.$email);
	  }
        return $result;
    }
	...
	function userinfo($user_id = null){
		...
		if ($check_user['expire'] < time() ){
			$check_user['membership'] = get_option('standard_membership');
		}
		
		return $check_user;
	}
	
	function add_points($type , $points = null){
		global $db;
		...
		if(!$points){
			if ( $type == 'post' ){
				...
				$new_points = $current_user['points'] + $membership['p_post'];
				$data = array("points" => $new_points);
				$db->update("users", $data, 'id='.$current_user['id']);
				
			} else if ( $type == 'comment' ){
			
				$new_points = $current_user['points'] + $membership['p_comment'];
				$data = array("points" => $new_points);
				$db->update("users", $data, 'id='.$current_user['id']);
			
			} else if ( $type == 'dare' ){
			
				$new_points = $current_user['points'] + $membership['p_dare'];
				$data = array("points" => $new_points);
				$db->update("users", $data, 'id='.$current_user['id']);
			
			} else if ( $type == 'challenge' ){
			
				$new_points = $current_user['points'] + $membership['p_challenge'];
				$data = array("points" => $new_points);
				$db->update("users", $data, 'id='.$current_user['id']);
	...
	function check_friends ($user_id , $friend_id ){
		global $db;
		
		$check_friends = $db->fetchRow("SELECT * FROM friends WHERE (friend_one='$user_id' OR friend_two='$user_id') AND (friend_one='$friend_id' OR friend_two='$friend_id')");
	...
	function is_online($user_id){
		...
		$is_online = $db->fetchOne("SELECT online FROM users WHERE id='$user_id'");
		
		$online_time = time() - 60 * 5;
		if ($is_online > $online_time){
			$is_online = 1;
		} else {
			$is_online = 0;
		}
	...
	function get_avatar($user_id , $img = false , $class = null , $id = null, $admin = null){
		global $db;
	...
	function rand_color($s_color = null){
	
		$var_color = rand(1,6);
	
		$color = "";
		
		if ( $var_color == 1 ) { $color = "#a7d155"; }
		if ( $var_color == 2 ) { $color = "#eada42"; }
		if ( $var_color == 3 ) { $color = "#e54e4b"; }
		if ( $var_color == 4 ) { $color = "#42AEEA"; }
		if ( $var_color == 5 ) { $color = "#C842EA"; }
		if ( $var_color == 6 ) { $color = "#EA9942"; }
	...
	function get_membership ($user_id){
		...
		if ( $userinfo['expire'] < time() ){
			...
			$get_membership = $db->fetchRow("SELECT * FROM memberships WHERE id = '".get_option('standard_membership')."'");
		
		} else {
		
			$get_membership = $db->fetchRow("SELECT * FROM memberships WHERE id = '".$userinfo['membership']."'");
	...
	function get_school($school_id){
		...
		$get_school = $db->fetchOne("SELECT name FROM schools WHERE id = '".$school_id."'");
	...
	function top_school_cofessions($amount = 5 , $type = 'widget', $id = null, $name = null, $counter = null  ) {
		...
		if ( $type == 'widget' ) {
			for ($x=0; $x < $amount; $x++) {
				if ($r = $db->fetch_array($results)) {
					if ( get_school($r['id']) == ""){
						...
						$output .= "<p class='author-data'><i class='col-2 fa fa-lg fa-bank'></i>".get_school($r['id']). " - ".$r['top_schools']."</p>";
					}
				}
			}
		} else {
			...
			for ($x=0; $x < $amount; $x++) {
				if ($r = $db->fetch_array($results)) {
					$output .= "{ label: '".get_school($r['id'])." - ".$r['top_schools']." $counter',data: ".$r['top_schools']."},";
				}
			}
