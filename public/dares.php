<?php
	...
	$publish_directly = '';
	if ( get_option('publish_dares_directly') == 1){
		$publish_directly = 1;
	} else {
		$publish_directly = 0;
	}
	...
	if( $input->pc['delete_dare'] ){		
		$db->delete("dares", 'id='.$input->pc['delete_dare']);
		$db->close();
	}
	...
	if( $input->pc['admin-ed'] ){
		...
		if ( $verifyuser == 1 ){
			...
			$insert = $db->update("dares", $data, "id=".$input->pc['admin-ed']);
		}
	}
	
	if( $input->pc['dare_add'] ){
	...
	if ( $verifyuser == 1 ) {
		...
		$paginator = new Pagination("dares", $conditions);
		...
		$count_dares = $db->fetchOne("SELECT COUNT(*) AS NUM FROM dares$conditions2 ORDER BY id ASC");
...
?>
  ...
  <div class="main clearfix">
    ...
    <div class="content">
        ...
        <div class="post new-post" style="display:none;">
          <div class="post-header">
			...
			<form method="post" class='sky-form' style="padding: 15px;" action="./index.php?view=dares">
				<input name="dare_add" type="hidden" value="true">
				<div class="row">
					<div class="col col-12">
						<label class="input">
							<i class="icon-append fa fa-font"></i>
							<input type="text" name="title" placeholder="Title (eg. I dare you to ...)">
						</label>
					</div>
				</div>
				<div class="row">
					<div class="col col-12">
						<label class="input">
							...
							<input type="text" name="user_b" placeholder="Dare a user ( Username ) | Optional">
						</label>
					</div>
				</div>
				<div class="row">
            ...
            <!-- .settings-field -->
				<div class="row">
					<button type="submit" class="button">Post It (Dare it)</button>
		...
		<div class="post admin-edit-dare" style="display:none;">
          <div class="post-header">
            <h2>Edit dare</h2>
          </div>

			<form method="post" class='sky-form' style="padding: 15px;" action="./index.php?view=dares">
				<input name="admin-ed" id="admin-edares" type="hidden" value="true">
				<div class="row">
					<div class="col col-12">
						<label class="input">
							<i class="icon-append fa fa-font"></i>
							<input type="text" name="title" id="admin-etitle" placeholder="Title (eg. I dare you to ...)">
						</label>
					</div>
				</div>
				<div class="row">
					<div class="col col-6">
						<label class="input">
							<i class="icon-append fa fa-user"></i>
							<input type="text" name="user_a" id="admin-eusera" placeholder="Dare By">
						</label>
					</div>
					<div class="col col-6">
						<label class="input">
							<i class="icon-append fa fa-user"></i>
							<input type="text" name="user_b" id="admin-euserb" placeholder="Dare a user ( Username ) | Optional">
						</label>
					</div>
				</div>
				<div class="row">
            ...
            <!-- .settings-field -->
				<div class="row">
					...
					<button type="submit" class="button">Save Dare</button>
					<button type="reset" class="button close-admin-dare">Cancel</button>
					<a class="button admin-delete-dare" data-id=''>Delete</a>
				</div>
	   ...
	   <div class="clearfix"></div>
		...
		<div style="display:none;" class="new-post td-info warning">
			...
			You have to be logged in to post a dare</p>
		<div class="close-btn"></div>
		</div>
		<?php }?>
      <div class="post" id="dare-append">
        <div class="post-header">
          <p class="results"><span> <?php echo $count_dares; ?> </span> Dares</p>
          <label class="sort">
            ...
            <tbody>
				...
				<tr class="opening" id="su<?php echo $r['id']; ?>">
					...
					<td class="opening-meta">
						<h2>
							<a class="opening-name take-dare" data-id="<?php echo $r['id']; ?>"><?php echo $r['title']; ?></a>
							<?php if ( $user_info['role'] == 1){ ?>
								- <a class="opening-name edit-dare" data-id="<?php echo $r['id']; ?>" data-title="<?php echo $r['title']; ?>" data-user_a="<?php echo $r['user_a']; ?>" data-username_a="<?php echo get_username($r['user_a']); ?>" data-user_b="<?php echo $r['user_b']; ?>" data-username_b="<?php echo get_username($r['user_b']); ?>" data-content="<?php echo $r['content']; ?>" data-proof="<?php echo $r['proof']; ?>" data-money="<?php echo $r['money']; ?>" data-finish="<?php echo $r['time_finish']; ?>" data-status="<?php echo $r['status']; ?>" data-opened="<?php echo date("M d, y",$r['date']); ?>"><i class="fa fa-edit"></i></a>
							<?php } ?>
						</h2>
						<div class="post-meta">
							<span class="post-author"><i class="fa fa-user"></i> <a href="./index.php?view=user&user-id=<?php echo $r['user_a']; ?>" title="Posts by <?php echo $current_user['username']; ?>" rel="author">Dare by <?php echo get_username($r['user_a']); ?></a></span>
						...
						<div class="post-meta">
							...
							<span class="post-author"><i class="fa fa-user"></i> <a href="./index.php?view=user&user-id=<?php echo $r['user_b']; ?>" title="Dare taked by <?php echo $current_user['username']; ?>" rel="author">Dare taked by <?php echo get_username($r['user_b']); ?></a></span>
							<span class="post-author"><i class="fa fa-history"></i> Dare Closed </span>
							<?php } else if ( $r['user_b'] != 0 && $r['status'] == 2 ) {?>
							<span class="post-author"><i class="fa fa-user"></i> <a href="./index.php?view=user&user-id=<?php echo $r['user_b']; ?>" title="Dare taked by <?php echo $current_user['username']; ?>" rel="author">Dare WON by <?php echo get_username($r['winner']); ?></a></span>
							<span class="post-author"><i class="fa fa-history"></i> Dare Closed </span>
							<?php } else if ( $r['user_b'] == 0 && $r['status'] == 1 ) {?>
							<span class="post-author"><i class="fa fa-history"></i> Opened Dare , take it </span>
							<?php } ?>
						</div>
					</td>
				</tr>
    ...
    <!-- .content -->
    <aside class="sidebar">
		<button id="add_post" class="peter-river-fb small-fb radius-fb" style="margin-bottom:10px;display: inline-block;width: 100%;box-sizing: border-box;">Add Dare</button>
      	<?php 
	...
	$(document).ready(
		...
		$(document).on('click', '.edit-img-proof', function(){
			...
			$(this).imgPicker({
				...
				obj_id: $(".appended-dare-info").data('id') ,
				complete: function(imageUrl) {
					// Set body background
					$('.new-proof').attr("src",imageUrl);
				}
			});
		...
		$( ".take-dare" ).click(function() {
		 ...
		 $(document).on('click', '.finish-dare', function(){
		
			var uform = $( "#working-form" );
		
			$.post( "index.php?view=moderate-dares", uform.serialize() );
			
			$( this ).fadeTo( "slow", 0.33 ).delay( 1200 ).fadeTo( "slow", 1 );
		});
		
		$(document).on('click', '.accept-dare', function(){
		...
		$( ".edit-dare" ).click(function() {
			...
			$( ".admin-delete-dare" ).data('id' , info.id)
			...
			$('.admin-edit-dare').slideDown(1500);
		});
		
		$( ".admin-delete-dare" ).click(function() {
			var d_info = $( this ).data();
			$('#su' + d_info.id).slideUp();
			$.post( "./index.php?view=dares", { delete_dare: d_info.id } );
			$('.admin-edit-dare').slideUp(1500);
		});
		
		$( ".close-admin-dare" ).click(function() {
			$('.admin-edit-dare').slideUp(1500);
		});
		<?php } ?>
		
		$( ".close-dare" ).click(function() {
			$('.appended-dare-info').slideUp(1500);
		});
		}
	);
	</script>
